...
const GeometricSecurityCore: React.FC<{ isReducedMotion: boolean }> = ({ isReducedMotion }) => {
  ...
  useFrame((_, delta) => {
    ...
    // Invalidate next frame on animation tick
    invalidate();
  });
...
// Scene Root with Demand Frameloop and Mouse Interaction Invalidation
const SceneRoot: React.FC<{ isReducedMotion: boolean }> = ({ isReducedMotion }) => {
  ...
  return (
    <>
      ...
      <Float speed={isReducedMotion ? 0 : 1.4} rotationIntensity={0.3} floatIntensity={0.6}>
        <GeometricSecurityCore isReducedMotion={isReducedMotion} />
      </Float>

      <FloatingDataParticles count={100} isReducedMotion={isReducedMotion} />
    </>
  );
};
...
export default function HeroScene() {
  ...
  return (
    <LazyCanvasWrapper persistent={true} className="w-full h-[400px] sm:h-[480px] lg:h-[540px]">
      {({ isReducedMotion }) => (
        <Canvas
          frameloop="demand"
